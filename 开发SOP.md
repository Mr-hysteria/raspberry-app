...
目标屏幕自报 EDID 名称为 `MPI5001`，对应 5 英寸 HDMI Display-B，原生分辨率为 `800×480`。这类面板会弱化接近白色的层次，且实体观感可能比开发机截图更冷、更亮。

先读取真实输出状态，不要仅凭仓库里的分辨率推断型号：

```bash
DISPLAY=:0 XAUTHORITY="${HOME}/.Xauthority" xrandr --verbose
```
...
- 当前模式为 `800×480 @ 60Hz`
- EDID 名称为 `MPI5001`
- `Broadcast RGB` 为 `Full`

`run-clock.sh` 会在每次启动时把 HDMI 输出设为 `Full RGB`。如果设备使用其他输出名，可覆盖：

```bash
RASPBERRY_CLOCK_DISPLAY_OUTPUT=<output-name> ./run-clock.sh
```

视觉设计与验收遵循以下顺序：

1. 本地预览检查布局、裁切和程序输出色值。
2. 部署后用 `scrot` 检查树莓派渲染结果，排除字体或渲染器差异。
3. 在实际书桌环境正面观察实体屏；必要时拍摄同环境照片比较。
4. 一次只改变一个变量，例如先测试 RGB 范围，再调整色板，避免无法判断改进来源。

注意：`scrot` 抓取的是帧缓冲，不包含背光、面板色域、可视角度和 OSD 设置造成的变化。暗环境中的手机照片还可能自动过曝和偏色，因此照片适合判断“层次是否消失”，不适合反推精确 RGB 数值。

针对当前 MPI5001 的稳定原则：

- 用深画布与中亮度纸页形成粗粒度明暗分离，不依赖多个相近浅米白。
- 画布文字与纸页文字使用独立色值，分别保证浅字/深底和深字/浅底。
- 关键边框和分隔线使用不透明色，实体屏上至少 `2px`；避免低透明度细线承担结构。
- 白天色板保持固定，日期只改变装饰位置，避免每天更换相近颜色后重新暴露面板偏差。
- 调色前先确认 OSD 亮度、对比度和色温没有被意外改变。
