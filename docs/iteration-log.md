...
- 确认 Debian 13 的经典版 `unclutter` 不支持 `--timeout`、`--fork` 长参数
- 改用 `-idle 1 -jitter 1 -root` 并保持后台运行
- 增加启动脚本回归测试和维护文档说明
...
## 2026-07-01 - 每日一句背景图格式兼容修复

### 目标

- 修复接口返回 JPEG、但缓存固定命名为 PNG 导致的背景图解码失败
- 避免历史背景图和中断产生的临时图片持续占用磁盘

### 完成项

- 根据文件签名识别 JPEG、PNG、WebP 和 GIF，不再信任 URL 或固定扩展名
- 候选图片经 Slint 解码验证后才替换当前缓存
- 新缓存和 JSON 成功落盘后，清理其他格式旧图及中断残留文件
- 图片失败时保留上一张有效缓存；当前图片不可解码时继续重试当天数据
- 保留 8 MiB 下载上限，并拒绝被上限截断的图片
...
- 日期与星期合并成一行
- 秒数进一步弱化并移至时间右上方
- 英文和中文金句重新建立主次层级，并限制底部文本宽度

### 影响范围

- `ui/clock.slint`
- `src/main.rs`
- `tests/ui-font-sizes.sh`
